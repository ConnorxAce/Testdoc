...
2. Loads FRA config, checks `<basePath>/Prepare_Rules/Output/` for **ASA device files** — filters for files whose lowercase stem starts with `fw`, `asa`, or `rmb-` (case-insensitive). If no ASA files found, logs "Deploy skipped: no ASA device files in Prepare_Rules/Output" and returns (prevents deploying when only non-ASA device output exists)
