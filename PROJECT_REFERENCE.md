...
│   ├── diff_engine.py         # Scalable config diff engine (byte-offset index, git-diff, CSV export)
│   └── ftd_prepare.py         # FTD ACL parsing, inventory, FMC JSON builder (1038 lines)
├── ssh/
│   ├── __init__.py
...
│   │   ├── credentials.py            # 2 routes (38 lines)
│   │   ├── cancel.py                 # 4 routes (56 lines)
│   │   ├── logs.py                   # 2 routes (55 lines)
│   │   ├── global_rules.py           # 1 route (23 lines)
│   │   ├── show_version.py           # 5 routes (421 lines)
│   │   ├── devices.py                # 12 routes + 2 helpers (1814 lines)
│   │   ├── backups.py                # 14 routes (1072 lines)
│   │   └── firewall_rules_auto.py    # 29 routes (1404 lines)
│   └── web/
│       ├── templates/
...
│   ├── ftd_policy_inventory.txt # FTD device-policy mapping with IPS/URL tags
...
| `app/api/global_rules.py` | 1 global-rules route (23 lines) | GET `/api/global-rules/inventory` |
...
| `app/api/firewall_rules_auto.py` | 29 FRA routes (1403 lines) | Deploy, configure, audit, scheduler, reports |
| `services/ftd_prepare.py` | FTD pipeline (1038 lines) | FTD ACL parse/inventory/merge, FMC JSON builder with objects section |
...
| `/api/firewall-rules-auto/deploy` (ASA-only) | POST | `{basePath, username, password, enableSecret}` | `{status, jobId}` | `firewall_rules_auto.js:deployRules()` — starts async deploy of ASA `.txt` files only; FTD `.json` files are ignored; credentials never stored; returns jobId for polling; writes `started` line to `DeployAuthLogs.log` |
...
| `data/deploy_credential_key.key` | Raw Fernet key | Persistent (auto-created) | **Dedicated** Fernet encryption key for deploy credential storage. Generated once via `Fernet.generate_key()` on first use. **Completely independent** from Flask session secret key. If lost, existing encrypted deploy credentials become unrecoverable. |
| `data/firewall_rules_auto_deploy_creds.enc` | Fernet-encrypted JSON | Persistent | Encrypted deploy credentials `{username, password, enable_secret, saved_at}`. Encrypted with `data/deploy_credential_key.key`. |
...
`server.py` imports and registers all 8 blueprints (lines 59-108):

| Blueprint Module | Line Count | Routes |
|------------------|-----------|--------|
| `app.api.credentials` | 38 | 2 |
| `app.api.cancel` | 56 | 4 |
| `app.api.logs` | 55 | 2 |
| `app.api.global_rules` | 23 | 1 |
| `app.api.show_version` | 421 | 5 |
| `app.api.devices` | 1,836 | 12 + 2 helpers |
| `app.api.backups` | 1,072 | 14 |
| `app.api.firewall_rules_auto` | 1,404 | 29 |
...
|--------|---------------|
| `utils.helpers` | `clean_command_output`, `prefix_to_netmask`, `is_valid_netmask` |
| `utils.file_helpers` | `get_cache_status`, `get_device_state_path`, `load_device_state`, `get_device_info_from_cache` |
| `parsers.output_parsers` | `parse_show_route_output`, `parse_show_version_output`, `parse_fxos_chassis_detail`, `extract_trailing_context` |
...
| `services.diff_engine` | `TMP_DIFF_FOLDER` |
| `ssh.client` | `execute_command_wait_prompt` |

#### Major Regions

| Lines | Module/Region | Purpose |
|-------|---------------|---------|
| 1-24 | Standard library imports | `os`, `json`, `time`, `threading`, `datetime`, `flask`, `secrets`, `ipaddress`, `sqlite3`, `shutil`, `re` |
| 26-57 | Extracted utility imports | `utils.helpers`, `utils.file_helpers`, `parsers.output_parsers`, `services.show_version_cache`, `ssh.client`, `services.diff_engine` |
...
#### 6.2.1 `app/api/credentials.py` (38 lines, 2 routes)
- **`POST /api/credentials`** — Save credentials to `SESSION_CREDS[sid]`
- **`POST /api/credentials/clear`** — Clear `SESSION_CREDS[sid]`

#### 6.2.2 `app/api/cancel.py` (56 lines, 4 routes)
- **`POST /api/data-collection/cancel`** — Cancel data collection job
- **`POST /api/firewall-inventory/cancel`** — Cancel firewall inventory job
- **`POST /api/path-finder/cancel`** — Cancel path finder test
- **`POST /api/failover-check/cancel`** — Cancel failover check job

#### 6.2.3 `app/api/logs.py` (55 lines, 2 routes)
- **`GET /api/logs`** — List session logs
- **`GET /api/logs/<log_id>`** — Get single log entry

#### 6.2.4 `app/api/global_rules.py` (23 lines, 1 route)
- **`GET /api/global-rules/inventory`** — Load firewall inventory for global rules

#### 6.2.5 `app/api/show_version.py` (421 lines, 5 routes)
- **`GET /api/show-version/<hostname>`** — Cache retrieval
- **`POST /api/show-version-collect`** — Collect from device
- **`POST /api/show-version-cache-missing`** — Check missing cache entries
- **`POST /api/generate-show-version-csv`** — CSV generation
- **`GET /api/show-version-inventory`** — Inventory listing

#### 6.2.6 `app/api/devices.py` (1836 lines, 12 routes + 2 helpers)
- `collect_single_device`, `check_single_device_failover` helpers
- Device listing, interfaces, collection, ping/tracer/route/nat, failover-check, preferred-role routes
...
#### 6.2.8 `app/api/firewall_rules_auto.py` (1404 lines, 29 routes)
...
| `save_test_log(log_entry)` | Writes test log JSON entry to `data/session_logs/` with timestamp-hostname-test_type filename |

**Dependencies:** `json`, `datetime`, `Path` from `pathlib`
...
| Class | Purpose |
|-------|---------|
| `JumpboxSessionManager` | Manages a single jumpbox SSH connection with device count tracking, keep-alive checks, and automatic reconnection. Created per batch operation with a `jumpbox_config` dict. Exposes `ensure_connection()`, `get_connection()`, `close()`, `reset_device_count()`, and `handle_device_with_reconnect()`. |

#### JumpboxSessionManager Methods

| Method | Purpose |
|--------|---------|
| `__init__(jumpbox_config)` | Stores jumpbox config, initializes `jumpbox_client = None` |
| `ensure_connection()` | Connects to jumpbox if `None`; reconnects if connection lost via `_is_connection_alive()`; returns `True` if a connection was made |
| `get_connection()` | Calls `ensure_connection()` then returns `self.jumpbox_client` |
| `close()` | Closes jumpbox connection and resets `jumpbox_client` to `None` |
| `reset_device_count()` | Resets `self.device_count` to 0 |
| `_is_connection_alive()` | Checks jumpbox transport is active via `transport.send_ignore()` |
| `_is_jumpbox_related_failure(error_message)` | Returns `True` if error message contains jumpbox-related keywords (connection, timeout, socket, auth, SSH, etc.) |
| `handle_device_with_reconnect(device_info, device_credentials, operation_func)` | Ensures jumpbox connection, creates a device connection via `connect_to_device_via_jumpbox()`, passes it to `operation_func(device_conn, device_info, device_credentials)`. On jumpbox-related failure, reconnects jumpbox and retries once. Cleans up device connection (channel + client) in `finally` block. |

#### Key Functions

| Function | Signature | Purpose |
|----------|-----------|---------|
| `connect_via_jumpbox` | `(jumpbox_config: dict) -> paramiko.SSHClient` | Connects to jumpbox using `host`, `port`, `username`, `password` from config dict; returns connected SSHClient |
| `connect_to_device_via_jumpbox` | `(jumpbox_conn: SSHClient, device_info: dict, device_credentials: dict) -> dict` | Full SSH connection through jumpbox: tries direct-tcpip channel forwarding first, falls back to shell-hop (`connect_to_device_via_jumpbox_shell`). FXOS always uses shell-hop. Returns `{"client": SSHClient, "channel": Channel, "transport": Transport}` |
| `connect_to_device_via_jumpbox_shell` | `(jumpbox_conn: SSHClient, device_info: dict, device_credentials: dict) -> dict` | Shell-based jumpbox hop with device-specific login sequences (ASA `enable`, FTD `login as:`, FXOS `connect module` + `connect fxos`). Returns `{"client": SSHClient, "channel": Channel}` |
| `run_commands_on_device` | `(device_channel: Channel, commands: list, device_type: str, steps: list = None) -> tuple` | Sends multiple commands through an open channel, collects output for each; returns `(raw_outputs, clean_outputs)` dicts |
| `execute_command_wait_prompt` | `(channel: Channel, command: str, device_type: str, timeout: int = 30) -> str` | Sends command and waits for device-specific prompt pattern; returns output text |
| `execute_single_command` | `(jumpbox_conn: SSHClient, jumpbox_config: dict, device_info: dict, command: str, device_type: str) -> str` | High-level one-shot: connect to jumpbox + connect to device + execute command + return output |
| `run_failover_check_on_device` | `(device_channel: Channel, device_type: str, steps: list = None) -> tuple` | Runs failover show commands (`show failover` for ASA, `show firewall` for FTD/FXOS); returns `(raw_outputs, clean_outputs)` dicts |
| `parse_failover_state` | `(device_type: str, output: str) -> str` | Parses `show failover` (ASA) or `show firewall` (FTD/FXOS) output for active/standby state; returns `"Active"`, `"Standby"`, `"Unknown"`, or raw state string |

**Dependencies:** `paramiko`, `time`, `re`, `socket`, `clean_command_output` (from `utils.helpers`)
**Imported by:** `server.py` (via `from ssh.client import ...`), `app/api/devices.py` (lazy imports within route functions)
...
| Function | Purpose |
|----------|---------|
| `get_job_dir(job_id)` | Returns `TMP_DIFF_FOLDER / job_id` — used by both scalable diff and legacy SQLite compare |
...
### 6.16 services/ftd_prepare.py

**File:** `services/ftd_prepare.py` (1038 lines)

#### Purpose

Parses structured FTD change-input files (table rows + device-prefixed ACL lines) and generates FMC-compatible JSON access-policy rules. Supports CHG_ID extraction from file content, FTD/FTC device routing, FQDN-to-URL conversion, bi-directional rule splitting, and IPS/URL inventory tags.

#### Entry Points

| Function | Purpose |
|----------|---------|
| `load_ftd_policy_inventory()` | Reads `config/ftd_policy_inventory.txt`, returns `{device_key: {"policy": str, "ips": dict\|None, "url": bool, "urlcat": str\|None}}`. Format: `KEY:POLICY [tags...]` with bracket-based tags |
| `process_ftd_lines(bp, ftd_lines_by_chg, inventory, chg_fqdn_map=None)` | Main pipeline: parses lines grouped by `CHG_ID`, merges rules, builds FMC JSON, writes `Prepare_Rules/Output/{device_key}.json` per device. When device has `urlcat`, splits URL-eligible rules into a separate `{device_key}_{urlcat}.json` output with `urlcat` as policy name. `chg_fqdn_map` is `{chg_id: set_of_fqdn_names}` from CHG table row destinations — registered into per-device `fqdn_registry`. |
| `build_fmc_policy(device_key, rules, inventory, fqdn_names=None, policy_name_override=None)` | Constructs FMC `AccessPolicy` JSON with optional `objects.urls` and `objects.networks` sections. `policy_name_override` sets the policy name directly (used for urlcat policies). |
| `merge_rules_into_one(parsed_rules, chg_id, fqdns=None, has_url=False, category="FirewallRulesAuto")` | Merges parsed rules into 1-2 FMC rule dicts (forward + reverse for bi-directional). Converts FQDNs to URL conditions when `has_url=True` + port 80/443. When called from urlcat-split path, URL rules and non-URL rules are merged separately. `category` is passed through to `_build_rule()` — set to `urlcat` value for URL rules so they carry the correct category name. |

#### Classification & Parsing

| Function | Purpose |
|----------|---------|
| `is_table_line(line)` | **Single check for all table content.** Returns CHG_ID string (data row), `''` (header/separator), or `None` (not table). Replaces three separate checks (`extract_chg_id_from_line` + `TABLE_HEADER_PATTERN` + `TABLE_SEPARATOR_PATTERN`). |
| `is_ftd_line(line)` | Returns `True` if line starts with `FTD`/`FTC` (case-insensitive) and contains `]` |
| `extract_chg_id_from_line(line)` | Extracts `CHG\d+` from table data rows like `CHG0449517\|172.16.22.75\|...` (called internally by `is_table_line`) |
| `extract_rule_text(line)` | Splits line at `]` into `(device_key, rule_text)` |
| `parse_ftd_rule(rule_text)` | Routes to `parse_access_list()`, `parse_object_network()`, or `parse_object_service()` based on prefix. **Applies `FTD_ACL_TRAILING_RE` globally** (all rule types) before dispatching. |
| `parse_access_list(rule_text)` | Parses `access-list` lines into `{action, protocol, srcNetworks, dstNetworks, srcPorts, dstPorts}` with IP/host/object/any support |
| `parse_object_network(rule_text)` | Parses `object network <name> <type> <value>` - returns `{type: "network_object", name, subType, value}`; FQDN entries get `skip: True` |
| `clean_ftd_acl_line(rule_text)` | Strips trailing `Information!...` text and `log` keyword from ACL lines |
| `classify_ftd_rule(rule_text)` | Returns `"Firewall rules"`, `"Delete rules"`, `"Firewall config"`, `"Delete config"` |
| `normalize_ftd_parsed_rule(parsed)` | Normalizes port values for per-line parsed rules before merging |

#### Inventory Format

Lines in `config/ftd_policy_inventory.txt`:

```
FTD4DA-PRI:PRIMARY_FTD ips(enable:Default IPS) url(yes)
FTD-DEV:DEV_FTD
```

Tags are space-separated, case-insensitive, and may appear in any order:

| Tag | Example | Effect |
|-----|---------|--------|
| `ips(enable:ProfileName)` | `ips(enable:Balanced IPS)` | Injects `intrusionPolicy` + `variableSet` into all rules for device; profile defaults to `"Default IPS"` if omitted |
| `ips(disable)` | `ips(disable)` | Explicitly disables IPS for this device (default) |
| `url(yes)` | `url(yes)` | Enables FQDN→URL conversion for destination port 80/443 rules |
| `url(no)` | `url(no)` | Disables URL conversion (default) |
| `urlcat(CategoryName)` | `urlcat(WebFiltering)` | Paired with `url(yes)`. URL-converted rules are written to a separate output file `{device_key}_{CategoryName}.json` with policy name `CategoryName`. Non-URL rules from the same CHG stay in the main policy with `_1` suffix. |

#### FQDN to URL Pipeline

| Function | Purpose |
|----------|---------|
| `_process_fqdn_urls(all_dst_networks, all_dst_ports, fqdns)` | Moves FQDN named objects from `destNetworks` to URL list when URL flag + web port |
| `fqdn_registry` | `defaultdict(set)` keyed by `device_key`. Populated from `object network <name> fqdn <value>` and bare `fqdn <name>` lines across all CHGs for each device |
| `_has_fqdn_web_port(parsed_rule, fqdns)` | Returns `True` if rule is an `access_rule` with FQDN destinations in the `fqdns` set AND destination ports include 80 or 443. Used by `urlcat` to split URL-eligible rules into a separate policy. |

#### Bi-Directional Rule Splitting

| Function | Purpose |
|----------|---------|
| `_extract_rule_data(parsed_rules)` | Collects unified src/dst networks/ports with deduplication across all parsed rules in a CHG group |
| `_split_by_direction(parsed_rules)` | Splits parsed rules into forward/reverse groups based on first rule's source network set |
| `_build_rule(name, comment, action, protocol, src_nets, dst_nets, src_ports, dst_ports, delete_actions, urls=None, category="FirewallRulesAuto")` | Builds a single FMC rule dict with optional `urls`, `_deleteCommands`, and `category` fields. `category` defaults to `"FirewallRulesAuto"`, set to `urlcat` value for URL-split rules. |

#### Output JSON Structure

Each `Prepare_Rules/Output/{device_key}.json`:
  ...
  "rules": [
    {
      "name": "CHG001",
      "action": "ALLOW",
      "sourceNetworks": {"objects": [{"type": "Network", "value": "10.0.0.1/32"}]},
      "destNetworks": {"objects": []},
      "destPorts": {"objects": [{"type": "ProtocolPortObject", "value": "443"}]},
      "logEnd": true,
      "urls": {"urls": [{"type": "Url", "url": "appdynamics.com"}]}
    }
  ],
  "defaultAction": {"action": "DENY"},
  "objects": {
    "urls": [{"name": "appdynamics.com", "url": "appdynamics.com", "type": "Url"}],
    "networks": [{"name": "fqdn.example.com", "fqdn": "fqdn.example.com", "type": "Network", "subType": "FQDN"}]
  }
}
```

The `objects` section is only present when the rules reference URL objects or FQDN network objects that need to be created on FMC before applying the policy.

When a device has `urlcat(CategoryName)` in its inventory entry, URL-converted rules are written to a separate output file `{device_key}_{CategoryName}.json` with policy name `CategoryName`. Non-URL rules from the same CHG stay in the main policy file with `_1` suffix added to their rule name.

#### Flow in `api_fra_prepare()` (firewall_rules_auto.py)

1. Each input file is read line-by-line
2. **`is_table_line(line)`** — single check for ALL table content (data rows, headers, separators). Returns CHG_ID (set `current_chg_id` + extract FQDN from dest column into `chg_fqdn_map`), `''` (skip), or `None` (not table → continue to routing)
3. **`is_ftd_line(line)`** routes to `ftd_lines_by_chg[chg_id].append(line)` or ASA bucket
4. FQDNs extracted from CHG table row destination column (`CHG_ID\|Source\|Dest\|...`) are accumulated in `chg_fqdn_map = {chg_id: set_of_fqdn_names}` across all input files
5. ASA lines are written to temp files and go through `_fra_parse_input_files_for_prepare()` (existing path)
6. FTD lines + `chg_fqdn_map` are passed to `process_ftd_lines()` which:
   - Registers CHG-table FQDNs from `chg_fqdn_map` into per-device `fqdn_registry` alongside `fqdn` declaration lines
   - Extracts device key and rule text
   - Registers FQDN declarations from `object network <name> fqdn <value>` and bare `fqdn <name>` lines
   - Parses ACL rules via `parse_access_list()`
   - Merges per-CHG rules, detects bi-directional overlap, converts FQDNs
   - Builds policy JSON with `build_fmc_policy()`
   - Writes to `Prepare_Rules/Output/{device_key}.json`
7. Stats are aggregated across both ASA and FTD pipelines

#### Dependencies

`re`, `json`, `ipaddress`, `pathlib.Path`, `collections.defaultdict`, `utils.helpers.is_valid_netmask`

#### Imported by

`app/api/firewall_rules_auto.py` (lazy imports in `api_fra_prepare()`)
...
| 2026-06-01 | FTD pipeline: process_ftd_lines() groups lines by CHG_ID, builds per-device FMC JSON; fqdn_registry captures FQDN declarations from object network + bare fqdn lines; parse_object_network() fixed to preserve named objects in networks arrays; _process_fqdn_urls converts FQDNs to URL conditions when has_url + port 80/443; bi-directional detection in merge_rules_into_one produces CHG_ID/_1 rules; build_fmc_policy() emits objects.urls/objects.networks sections; inventory format extended with IPS Enable:Profile and URL True tags | `services/ftd_prepare.py` (new), `app/api/firewall_rules_auto.py`, `config/ftd_policy_inventory.txt` (new) | `/api/firewall-rules-auto/prepare` (FTD routing via is_ftd_line()) | None (all FTD pipeline runs transparently during prepare) |
| 2026-06-01 | Doc update — added ftd_prepare.py to repo tree, file inventory, per-file ref §6.16, and diff log; updated firewall_rules_auto.py line count (1306→1403); added ftd_policy_inventory.txt to config/ listing | `docs/PROJECT_REFERENCE.md` | None (doc-only changes) | None |
| 2026-06-02 | Redesigned inventory format from colon-delimited (`:IPS Enable:Profile :URL True`) to bracket-based (`ips(enable:Profile) url(yes)`); updated `INVENTORY_LINE_PATTERN` and added `INVENTORY_TAG_PATTERN` for space-separated tag parsing; changed `fqdn_registry` from `defaultdict(lambda: defaultdict(set))` (per-key, per-CHG) to `defaultdict(set)` (per-key only, cross-CHG); fixed `load_ftd_policy_inventory()` to parse new bracket-based tags | `services/ftd_prepare.py`, `config/ftd_policy_inventory.txt`, `docs/PROJECT_REFERENCE.md` | `/api/firewall-rules-auto/prepare` (via `is_ftd_line()` routing, `load_ftd_policy_inventory()`, `process_ftd_lines()`) | None |
| 2026-06-02 | Added `urlcat(CategoryName)` inventory tag — paired with `url(yes)`; moves URL-converted rules to a separate FMC access policy JSON file `{device_key}_{CategoryName}.json` with policy name `CategoryName`; non-URL rules from the same CHG stay in the main policy with `_1` suffix. Added `_has_fqdn_web_port()` helper to detect URL-eligible parsed rules for pre-merge splitting in `process_ftd_lines()`. Updated `build_fmc_policy()` with `policy_name_override` parameter. | `services/ftd_prepare.py`, `config/ftd_policy_inventory.txt`, `docs/PROJECT_REFERENCE.md` | Same as above | None |
| 2026-06-02 | Simplified line classification: added `is_table_line()` replacing 3 separate checks (`extract_chg_id_from_line` + `TABLE_HEADER_PATTERN` + `TABLE_SEPARATOR_PATTERN`) — single function returns CHG_ID (data row), `''` (header/separator), or `None` (not table). Extracted FQDN names from CHG table row destination column (`CHG_ID\|Source\|Dest\|...`) registered via new `chg_fqdn_map` parameter to `process_ftd_lines()`. Fixed `files_scanned` counter (was missing after refactor). Moved `import ipaddress` from inline try-block to function-level import. Fixed `parse_ftd_rule()` to apply `FTD_ACL_TRAILING_RE` globally before dispatching (not just access-list). Fixed `fqdn` parser to handle no-space case (`fqdnname` → `rule_text[4:].strip()`). Fixed `object network` fallback to mark as `skip: True`. | `services/ftd_prepare.py`, `app/api/firewall_rules_auto.py`, `docs/PROJECT_REFERENCE.md` | `/api/firewall-rules-auto/prepare` (line classification + FQDN extraction from table rows) | None |
| 2026-06-02 | Renamed deploy button: "Deploy Rules" → "Deploy ASA Rules" (button + modal title). Confirmed deploy/schedule workflows are fully isolated from FTD `.json` files: `_fra_deploy_worker` globs only `*.txt`, `_fra_scheduled_deploy` explicitly filters for `fw/asa/rmb-` stems. | `app/web/templates/index.html`, `docs/PROJECT_REFERENCE.md` | None (UI label only) | `#fra-deploy-btn`, `#fraDeployModalLabel` |
| 2026-06-02 | Added `category` parameter to `_build_rule()` and `merge_rules_into_one()` — default `"FirewallRulesAuto"`. URL rules split via `urlcat` now carry the category name matching their policy (e.g. `"WebFiltering"`) instead of the hardcoded `"FirewallRulesAuto"`. | `services/ftd_prepare.py`, `docs/PROJECT_REFERENCE.md` | `/api/firewall-rules-auto/prepare` (FTD urlcat output) | None |
...
**Deploy job (ASA-only)** (when `deployTime` matches):
1. Loads encrypted credentials from `data/firewall_rules_auto_deploy_creds.enc` and decrypts with Fernet using the **dedicated** key `data/deploy_credential_key.key` (completely independent from Flask session secrets).
   - **Migration:** If the dedicated key fails, falls back to the legacy Flask-secret-derived Fernet key. If legacy decrypt succeeds, credentials are automatically re-encrypted with the new dedicated key and the old file is overwritten — no manual intervention needed.
2. Loads FRA config, checks `<basePath>/Prepare_Rules/Output/` for **ASA device files** — filters for files whose lowercase stem starts with `fw`, `asa`, or `rmb-` (case-insensitive). If no ASA files found, logs "Deploy skipped: no ASA device files in Prepare_Rules/Output" and returns (prevents deploying when only FTD `.json` output exists)
...
7. After all devices processed, backs up `Prepare_Rules/Output/*.txt` and deploy logs to `Prepare_Rules/Backup/<timestamp>/` via `_fra_backup_prepare_rules()` — clears Output folder for next prepare cycle, also moves `prepare_audit.log`, `ExcludedLog.log`, `errors.log`
8. Records `lastDeployDate` to `Prepare_Rules/State/last_scheduled_deploy_date.txt` and in-memory guard; same-day duplicate prevents re-running
